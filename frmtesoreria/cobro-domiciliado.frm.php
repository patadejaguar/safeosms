...
//<=====	FIN_H
//=====================================================================================================
$xHP		= new cHPage("Cobranza .- DOMICILIA");
...
function jsaSetPago($Recibo, $cuentabancaria, $monto1, $diferencia, $fecha, $transaccion, $bancodeorigen, $referencia){
    $xF		= new cFecha(); 
    $fecha	= $xF->getFechaISO($fecha);
    $xCaja	= new cCaja(); 
    $xCaja->setReferenciaTrans($referencia);
    
    $op		= $xCaja->setCobroDomiciliado($Recibo, $cuentabancaria, $monto1, $diferencia, $fecha, "", false, false, $transaccion, $bancodeorigen);
...
$jxc ->exportFunction('jsaSetPago', array('iRecibo', 'iBancos', 'iMonto', 'iDiferencia', 'idfecha-0', "idtipodeperfil", "idcodigodebanco", "idreferencia"), '#avisos' );
$jxc ->process();
$xHP->init();
...
if( setNoMenorQueCero($recibo) >0 ){
    ...
    $xFRM	= new cHForm("frmcobrodomicilia", "cobro-domiciliado.frm.php");
    $xTxt	= new cHText("id");
    $xDat	= new cHDate();
    $xHSel	= new cHSelect();
    $xHNot	= new cHNotif();
    ...
    $xFRM->ODate("idfecha-0", $xRec->getFechaDeRecibo());
    $xFRM->OText_13("idreferencia", "", "TR.REFERENCIA");
    ...
    //$xFRM->addHElem( $xTxt->getNormal("iddocumento", "", "TR.REFERENCIA") );
