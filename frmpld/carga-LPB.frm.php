...
//=====================================================================================================
$xHP		= new cHPage("Carga Lista de Personas Bloquedas CNBV", HP_FORM);
$xQL		= new MQL();
$xLi		= new cSQLListas();
$xF			= new cFecha();
...
$xFRM		= new cHForm("frmcargaplb", "carga-LPB.frm.php?action=" . MQL_ADD);
$xSel		= new cHSelect();
...
if($action == SYS_NINGUNO){
	...
	$xFRM->OCheck("TR.GUARDAR COMO PERSONAS", "idguardarpersonas");
	...
	$xAmlLI				= new cAMLListaNegraInterna();
	...
	foreach($data as $valores => $cont){
		$idint			= $cont[0];
		$nombre			= $xFil->cleanString($cont[1], '"');
		$rfc			= $cont[2];
		
		if(setNoMenorQueCero($idint)>0){
			$idinterno	= "LPB-" . $idint .  "-" . $rfc;
			$xAmlLI->addLPB($idinterno, $nombre, $observaciones);
		}
	}
	
	$xFRM->addLog($xAmlLI->getMessages());
