...
$runcmd			= parametro("idarchive", ""); $runcmd	= setNoMenorQueCero($runcmd);
...
switch ($action){
	case SYS_NINGUNO:
		...
		$xFRM->addHElem($xSel->get("idarchive", "TR.OPCION"));
		$xFRM->addSubmit();
		$xFRM->setAction("../tools/migrate-gpd1601.frm.php?action=". MQL_MOD);
	...
	case MQL_MOD:
		...
		if($runcmd == "personas"){
         ...
         IF(getMigFigJur(`personalidad_juridica`) = 'M', '', `socios_general`.`genero`) AS `genero`,
         ...
         IF(getMigFigJur(`personalidad_juridica`) = 'M', '',`socios_general`.`curp`) AS `curp`,
         ...
         IF(getMigFigJur(`personalidad_juridica`) = 'M', '',`socios_general`.`tipo_de_identificacion`) AS `tipo_identificacion_id`,
         `socios_general`.`regimen_fiscal` AS `regimen_fiscal_id`,
         ...
         `socios_general`.`regimen_conyugal` AS `regimen_matrimonial_id`,
         ...
         `socios_general`.`estadocivil` AS `estado_civil_id`,
	...
	IF(getMigEsCliente(`socios_general`.`codigo`)='true', 'true', '') AS `cliente`,
         `nivel_de_riesgo_aml` AS `nivel_riesgo_pld_id`,
...
INNER JOIN `t_03f996214fba4a1d05a68b18fece8e71`  ON `socios_general`.`idusuario` = `t_03f996214fba4a1d05a68b18fece8e71`.`idusuarios`";
			...
			class cTmp {
				...
				public  $curp	= '';
				public  $observacion	= '';
				public  $ocupo_cargo_publico	= '';
				public  $nacionalidad_extranjera	= '';
				public  $empresa_empleadora	= '';
				public  $rfc	= '';
				public  $firma_electronica	= '';
				public  $razones_no_firma	= '';
				public  $razon_social	= '';
				public  $pais_id	= '';
				public  $tipo_de_identificacion_id	= '';
				public  $regimen_fiscal_id	= '';
				public  $estado_id	= '';
				public  $telefono	= '';
				public  $activo	= '';
				public  $regimen_matrimonial_id	= '';
				public  $tipo_persona	= '';
				public  $numero_acta	= '';
				public  $empresa_id	= '';
				...
				public  $fecha_vencimiento_carnet	= '';
				public  $propiedades_en_extranjero	= '';
				public  $cuenta_contable_id	= '';
				public  $cliente	= '';
				public  $nivel_riesgo_pld_id	= '';
				public  $nombre_razon	= '';
				public  $ha_estado_en_cartera_judicial	= '';
			}
