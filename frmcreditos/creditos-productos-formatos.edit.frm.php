...
//=====================================================================================================
$xHP		= new cHPage("TR.PRODUCTO FORMATOS", HP_FORM);
$xQL		= new MQL();
$xLi		= new cSQLListas();
$xF			= new cFecha();
...
/* ===========        FORMULARIO EDICION         ============*/
$xTabla        = new cCreditos_prods_formatos();
$xTabla->setData( $xTabla->query()->initByID($clave));
$xFRM    = new cHForm("frmiddividdivcredsprodsforms", "creditos-productos-formatos.frm.php?action=$action");

if($clave<=0){
	if($producto>0){
		$xTabla->producto_credito_id($producto);
	}
	$xTabla->idcreditos_prods_contratos("NULL");
}

$xFRM->setTitle($xHP->getTitle());
$xSel        = new cHSelect();

$xFRM->OHidden("idcreditos_prods_contratos", $xTabla->idcreditos_prods_contratos()->v());

$xFRM->addHElem( $xSel->getListaDeFormatos("formato_id",$xTabla->formato_id()->v())->get(true) );
