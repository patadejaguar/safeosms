...
if( $xRec->init() == true){
	...
	*/
	$xT5	= new cTabla( $xL->getListadoDeEventos("", "", "", "", "", "", false, false, $recibo));
	$xT5->setTipoSalida($out);
	$xRPT->addContent($xT5->Show("TR.Eventos"));
	...
	if($xRec->getEsEliminado() == true){
		$xQL	= new MQL();
		$data	= $xQL->getDataRow("SELECT * FROM `operaciones_recibos_arch` WHERE `idoperaciones_recibos`=$recibo LIMIT 0,1");
		...
		$cEdit		= new cTabla($xL->getListadoDeOperacionesArch("", "", $recibo));
