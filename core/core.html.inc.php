...
class cHPage {
	...
	function goToPageError($iderror = DEFAULT_CODIGO_DE_ERROR, $out = OUT_HTML){
		...
		//caja cerrada
		if($iderror == 200){
			$ff = $_SERVER['REQUEST_URI']; //$ff = parse_url($_SERVER['REQUEST_URI']);
...
afterTagRemoved: function(event, ui){ var xF=new FrmGen();var mtag = ui.tag[0].innerText; $('#$id').val(xF.tagRem(mtag,'$id')); } }); }";
	...
	function OEntero($id, $valor, $titulo,$maximo = 999999){
		$xTxt	= new cHText();
		$xTxt->setDivClass("tx4 tx18");
		$this->addHElem( $xTxt->getDeConteo($id, $titulo, $valor, $maximo) );
	...
	function OTasaInt($id, $valor, $titulo=""){
		$titulo	= ($titulo == "") ? "TR.TASA" : $titulo;
		$xTxt	= new cHText();
		$xTxt->setDivClass("tx4 tx18");
		$this->addHElem( $xTxt->getDeTasaInt($id, $titulo, $valor) );
		$xTxt	= null;
		return $xTxt;
	}
	...
	function OFileDoctos($id , $valor = "", $titulo = ""){
		$xFil		= new cHFile();
		$xFil->setProperty("accept", "image/jpeg,image/png,application/pdf");
		$xFil->setNoCleanProps();
		$this->addHElem( $xFil->getBasic($id, $valor, $titulo) );
		$this->setEnc("multipart/form-data");
	}
	...
	function addSeccion($id, $titulo, $btn=""){
		...
		if($this->mIsWizard == true){ 
			...
			$this->addHElem("<h3 id=\"h3$id\"><a href=\"#\">$titulo</a>$btn</h3><div class='formoid-default formoid-section' id='$id'>");
		}
	...
	function setNoAcordion($res=true){ $this->mAccLock	= $res; }
  ...
  slider.each(function(){
	...
	function addBuscar($evt){
		$this->OButton("TR.BUSCAR", $evt, $this->ic()->BUSCAR, "idcmdbuscarx", "blue");
	}
	function addTag($txt, $tipo = "", $evt=""){
		$xNot		= new cHNotif();
		if($tipo == "err" OR $tipo == "error"){
			$tipo	= $xNot->ERROR;
			
		}
		$this->addHElem( $xNot->getTag($txt, "", $tipo, $evt) );
	}
...
class cHText extends cHInput {
	...
	function getDeTasaInt($id = "", $Titulo = "", $value = 0){
		$this->setClearProperties();
		//$this->setClearEvents();
		$ctrl						= "";
		$id							= ( $id == "" ) ? $this->mId : $id;
		...
		$this->mArrProp["name"]		= $id; $this->mLIDs[]	= $id;
		$this->mArrProp["value"]	= $value;
		$this->mArrProp["step"]		= "0.001";
		
		$this->mArrProp["maxlength"]= 10;
		$this->mArrProp["max"]		= "999";
		$this->mArrProp["min"]		= "0";
	...
	function getDeTelefono($id = "", $Titulo = "", $value = 0){
		$this->setClearProperties();
		$Titulo						= ($Titulo == "") ? "TR.TELEFONO" : $Titulo;
		$ctrl						= "";
		//$ln							= strlen("$Maximo")+1;
		$id							= ( $id == "" ) ? $this->mId : $id;
		$this->mArrProp["type"]		= "tel";
		//$this->mArrProp["class"]	= "";
		$this->mArrProp["name"]		= $id; $this->mLIDs[]	= $id;
		$this->mArrProp["value"]	= $value;
		$this->mArrProp["step"]		= "1";
		$this->mArrProp["maxlength"]= "10";//9811098164
		$this->mArrProp["minlength"]= "7";//9811098164
...
class cHSelect {
	...
	function getListaDeProductosDeCreditoXEmpresa($id = "", $empresa, $selected = false){
		$id		= ($id == "") ? "idproducto" : $id; $this->mLIDs[]	= $id;
		$selected	= setNoMenorQueCero($selected);
		$empresa	= setNoMenorQueCero($empresa);
		$sqlSc		= "SELECT   `creditos_tipoconvenio`.`idcreditos_tipoconvenio`, `creditos_tipoconvenio`.`descripcion_tipoconvenio`
			FROM     `socios_aeconomica_dependencias` 
			INNER JOIN `creditos_tipoconvenio`  ON `socios_aeconomica_dependencias`.`producto_preferente` = `creditos_tipoconvenio`.`idcreditos_tipoconvenio` 
			WHERE    ( `socios_aeconomica_dependencias`.`idsocios_aeconomica_dependencias` = $empresa ) AND (`idcreditos_tipoconvenio` !=99) AND `tipo_en_sistema`=" . SYS_PRODUCTO_NOMINA . " AND (`creditos_tipoconvenio`.`estatus` != 'baja')
			
			UNION
			SELECT   `creditos_tipoconvenio`.`idcreditos_tipoconvenio`, `creditos_tipoconvenio`.`descripcion_tipoconvenio`
			...
			WHERE    ( `creditos_solicitud`.`persona_asociada` = $empresa ) AND ( `creditos_solicitud`.`saldo_actual` >0.99  ) AND (`idcreditos_tipoconvenio` !=99) AND `tipo_en_sistema`=" . SYS_PRODUCTO_NOMINA . " AND (`creditos_tipoconvenio`.`estatus` != 'baja')
			GROUP BY `creditos_solicitud`.tipo_convenio";
		$xS 		= new cSelect($id, $id, $sqlSc);
		$xS->setLabel("TR.Producto de Credito");
		if($selected > 0){ $xS->setOptionSelect($selected); }
		$xS->setEsSql();
	...
	function getListaDeProductosDeCreditoGrupos($id = "", $selected = false){
		$id		= ($id == "") ? "idproducto" : $id; $this->mLIDs[]	= $id;
		$selected	= setNoMenorQueCero($selected);
		$sqlSc		= "SELECT `idcreditos_tipoconvenio`, CONCAT(`descripcion_tipoconvenio`,' .- ',`idcreditos_tipoconvenio`, '') AS `descripcion`
					FROM `creditos_tipoconvenio` WHERE	(`idcreditos_tipoconvenio` !=99) AND `tipo_en_sistema`=" . SYS_PRODUCTO_GRUPOS . " AND (`estatus` != 'baja') ORDER BY `descripcion_tipoconvenio` ";
		$xS 		= new cSelect($id, $id, $sqlSc);
		$xS->setLabel("TR.Producto de Credito");
		if($selected > 0){ $xS->setOptionSelect($selected); }
		$xS->setEsSql();
	...
	function getListaDeOperacionesPorBase($base, $id = "", $base2 = false, $selected = false, $activos = false){
		...
		if($activos == true){
			 $ByBase	= ($ByBase == "") ? " WHERE (`operaciones_tipos`.`estatus`=1) " : "$ByBase AND (`operaciones_tipos`.`estatus`=1) ";
		}
	...
	function getListaDeOperacionesAports($id = "", $selected = 0){
		$id			= ($id == "") ? "idtipodeoperacion" : $id; $this->mLIDs[]	= $id;
		$selected	= setNoMenorQueCero($selected);
		$sqlSc		= "SELECT   `operaciones_tipos`.`idoperaciones_tipos`, `operaciones_tipos`.`descripcion_operacion`, `personas_aports_tipos`.`estatusactivo`
		FROM     `personas_aports_tipos` INNER JOIN `operaciones_tipos`  ON `personas_aports_tipos`.`operacion_tipo_id` = `operaciones_tipos`.`idoperaciones_tipos` WHERE    ( `personas_aports_tipos`.`estatusactivo` = 1 )";
		$xS 		= new cSelect($id, $id, $sqlSc);
		if($selected > 0){
			$xS->setOptionSelect( $selected );
		}
	...
	function getListaDeCajasAbiertas($id = "", $tipo = "", $fecha = false){
		...
		if($xUsr->getPuedeCerrarCajas() == false){
	...
	function getListaDeEstadoMvtosDeCtasBancs($id = "", $agregar_especiales = false, $select = false){
		$id		= ($id == "") ? "idestatusobancs" : $id; $this->mLIDs[]	= $id;
		
		$select	= ($select == false) ? SYS_TODAS : $select;
		$cCta 	= new cSelect($id, $id, "");
		$cCta->setNoMayus();
		$cCta->addTodas();
		$cCta->addEspOption("autorizado", "TR.AUTORIZADO");
		$cCta->addEspOption("noautorizado", "TR.NO AUTORIZADO");
		$cCta->addEspOption("cancelado", "TR.CANCELADO");
		
		$cCta->setOptionSelect($select);
		
		$cCta->setEsSql();
		$cCta->setLabel("TR.ESTATUS");
	...
	function getListaDeCreditosPorPersona($persona, $id, $select = false, $omitir =""){
		$xQL		= new MQL();
		$xLi		= new cSQLListas();
		$persona	= setNoMenorQueCero($persona);
		$omitir		= (trim("$omitir") == "") ? "": " AND FIND_IN_SET(`creditos_solicitud`.`numero_solicitud`, '$omitir')<=0 ";
		$sql 		= $xLi->getListadoDeCreditos($persona, false, false, false, $omitir);
		$rs			= $xQL->getDataRecord($sql);
...
//		$xS->addEspOption("0", "0");
	...
	function getListaDeBasesDelSis($id = "", $selected = 0, $tipo = "", $real=false, $inactivos = false){
		$id			= ($id == "") ? "idbase" : $id; $this->mLIDs[]	= $id;
		$ByStat		= ($tipo == "" OR $tipo == SYS_TODAS) ? "" : " AND (`tipo_de_base` = '$tipo' ) ";
		$ByAct		= ($inactivos == true) ? "" : " AND (`estatus` = 1) ";
		$SqlRpt		= "SELECT `codigo_de_base`, CONCAT(`codigo_de_base`, '-',`descripcion`) AS `descripcion` FROM `eacp_config_bases_de_integracion` WHERE (`codigo_de_base`>0) $ByStat $ByAct";
...
class cTabla {
	...
	private $mFieldSum2		= array();		//Array assoc
	...
	*/
	...
	function OInput($campo, $tipo="text",$preid = "", $props=""){
		$preid					= ($preid == "") ?  $campo : $preid;
		
		$this->mArrCtrl[$campo]	= "<input id=\"$preid-" . HP_REPLACE_ID . "\" value=\"" . HP_REPLACE_DATA . "\" type=\"$tipo\" $props />";
	}
	function ODicIcons(){
		...
		if($this->mSql!=""){
			...
			while ($rw = $rs->fetch_array()){
						...
						if($isCtrl == true){
							...
							if($isTiny == true){
								if($isNumero == false){
									...
									if($this->mPrepareChart == false AND strlen($currVal) > $this->mMaxRed){
										$ttInfo		= $this->mEsq[$nombre]["NNN"];
										//$currVal	= "<details><summary>" . substr($currVal, 0,$this->mMaxRed) . "</summary>". substr($currVal,$this->mMaxRed) . "</details>";
										$currVal	= "<img class='x24 button-mini' src='../images/ellipsis.png' onclick=\"var xG=new Gen();xG.informar({b64:true, msg: '" . base64_encode($currVal) . "', title:'$ttInfo'});\" />";
									}
								} else {
									if($this->mPrepareChart == false){
										$currVal	= "<details><summary></summary>". $currVal . "</details>";
									}
								}
							}
						}
				...
				if($this->mSubsEnable	== true){
					...
					if(isset($subHTML[$rw[$this->mKeyField]])){
						$icx22			= $ifl+2;
						$tds 			.= "<tr id=\"tr-subs-" . $this->mTbl . "-".str_replace(" ", "_", $rw[$this->mKeyField]) . "\"><td class='notice' colspan='$icx22'>" . $subHTML[$rw[$this->mKeyField]] . "</td></tr>";
						unset($subHTML[$rw[$this->mKeyField]]);
					}
...
class cSelect{
	...
	function addVacio($select = false){
		$this->addEspOption("", "-");
		if($select == true){
			$this->setOptionSelect("");
		}
	}
	function addTodas($select = false){
		$this->addEspOption(SYS_TODAS, SYS_TEXTO_TODAS);
		if($select == true){
			$this->setOptionSelect(SYS_TODAS);
		}
	}
...
class cHDiv {
	...
	function get($cnt = "", $label = "", $id =""){
		...
		$props		= "";
		if($this->mClickEvt !== ""){
			$props	.= " onclick=\"" . $this->mClickEvt . "\" ";
		}
		return "<div class='" . $this->mDivClass . "' $pid$props>$label$cnt</div>";
	}
	function addHElem($html){ $this->mHTML	.= $html; }
	function addHLinea($html){ $this->mHTML	.= "<div class='tx1'>$html</div>"; }
	function addClickEvt($evt){ $this->mClickEvt = $evt; }
}
...
class cHFormatoRecibo {
	...
	function render($IncPrint = true){
		...
		if($xRec->init() == true){
			...
			//checar si existe en una empresa
			//if($xRec->getEsDeEmpresa() == true){}
			//
...
class cFormato {
	...
	function setProcesarVars($arrVars = false){
		...
		if($this->mEsRecibo == true){
			...
			$head			= "<thead><tr>";
			$h2				= "";
			//XXX: Recordar para que mierda era esto.
			foreach ($DEsq as $key => $idx){
	...
	function setRecibo($recibo){
		...
		if($xRec->init() == true){
			...
			$this->mArr["variable_observacion_del_recibo"] 		= str_replace("---", " - ", $xRec->getObservaciones());
			$this->mArr["variable_monto_del_recibo_en_letras"]	= $xCant->letras();
			$this->mArr["variable_monto_del_recibo"] 			= $xCant->moneda();
...
/*			$this->mArr["vars_pers_ae_dom_calle"]					= "";
	...
	function setOriginacionLeasing($clave){
		...
		//Subtotal Iniciales
		$inicialSub		= $xT->monto_anticipo()->v() + $xCredOrg->getMontoComision() + $xT->monto_gestoria()->v() + $xCredOrg->getSeguroInicial() + $xT->monto_notario()->v();
		$inicialSub		+= $xT->renta_proporcional()->v();
		...
		foreach ($rs as $rw){
			...
			$residual		= $xCalc->getValorResidual($xCredOrg->getMontoVehiculo(), $xCredOrg->getMontoAliado(), $pzo, $TasaResPzo, $xCredOrg->getMontoAnticipo(true), $xCredOrg->getEsAdministrado() );//  $xT->monto_residual()->v();
			...
			$pzoresidual												= $xCalc->getValorResidual($xT->precio_vehiculo()->v(), $xT->monto_aliado()->v(), $pzo, $tasares, $xCredOrg->getMontoAnticipo(true), $xCredOrg->getEsAdministrado());
			...
			$pp_subtotal		= $pp_subtotal_iva + $RentaEnDep; 					//Suma con renta de deposito
			...
			if($SumarRenta == false){
				...
				//setError("$pp_subtotal - $pp_renta_disminuida");
			}
...
class cCatalogoOperacionesDeCaja {
	 ...
	 * @return integer Tipo de Operacion
	 */
