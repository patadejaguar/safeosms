...
//=====================================================================================================
$xHP		= new cHPage("TR.Crear por Base", HP_FORM);
$xQL		= new MQL();
$xLi		= new cSQLListas();
$xF			= new cFecha();
...
$xFRM		= new cHForm("frmaportatiposporbase", "./aportaciones-tipos-por-base.frm.php?action=" . MQL_ADD);
$xSel		= new cHSelect();
$xFRM->setTitle($xHP->getTitle());

if($action == SYS_NINGUNO){
	$xFRM->addSubmit();
	$xFRM->addHElem( $xSel->getListaDeBasesDelSis("tipo",0, $xB->TIPO_OPERACIONES )->get(true) );
} else {
	$xFRM->addCerrar("", 3);
	if($tipo>0){
		$xB->setClave($tipo);
		if($xB->init() == true){
			$data	= $xB->getMembers_InArray();
			
			foreach ($data as $idx => $vv){
				$xTabla		= new cPersonas_aports_tipos();
				
				$xTabla->operacion_tipo_id($vv);
				$xTabla->personas_aports_tipos_id("NULL");
				$xTabla->estatusactivo(SYS_UNO);
				$xTabla->pagadero(CREDITO_TIPO_PERIOCIDAD_MENSUAL);
				$xTabla->tasa_rendimiento(0);
