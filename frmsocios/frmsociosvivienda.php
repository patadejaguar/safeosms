...
$xFRM		= new cHForm("frmvivienda", "frmsociosvivienda.php?action=" . MQL_ADD . "&persona=$persona");
...
if($xSoc->init() == true){
	...
	if($action == MQL_MOD){
		...
		$ready				= false;
		if(trim($calle) == "" OR trim($nexterior) == ""){
			$xLog->add("ERROR\tEl domicilio debe tener al menos CALLE / ACCESO / DIRECCION y un NUMERO EXTERIOR\r\n");
		} else {
			if(MODULO_AML_ACTIVADO == false AND $idlocalidad<= 0){
				$idlocalidad	= $xLoc->DomicilioLocalidadClave();
			}
		...
		if($isEdit == true){
			...
			$xFRM->setAction("frmsociosvivienda.php?action=" . MQL_MOD . "&persona=$persona&clave=$clave", true);
			//$xFRM->addActualizar();
			$xFRM->addGuardar();
		} else {
			$xFRM->addGuardar();
