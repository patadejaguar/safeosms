...
define("CREDITO_PRODUCTO_GRUPOS", 		900);		//REVISAR  julio2013
define("CREDITO_DEFAULT_DESTINO",		100);

define("CREDITO_PRODUCTO_DESTINO_DESCARTADOS",	203);
...
define("CREDITO_PRODUCTO_INDIVIDUAL",		200);
define("CREDITO_PRODUCTO_REVOLVENTES",		300);
define("CREDITO_PRODUCTO_NOMINA",		100);
define("CREDITO_PRODUCTO_FUERA_NOMINA",		201);
...
//define("FACTURACION_MAIL_CLAVE",			$xC->get("facturacion.clave_de_email_de_almacenamiento", "", MMOD_OPERACIONES));


define("OPERACION_CLAVE_DE_COBRANZA",		601);
define("OPERACION_CLAVE_DE_CARGOS_VARIOS",	600);
...
define("OPERACION_CLAVE_PAGO_COM_VARIAS",	146);
...
define("TESORERIA_COBRO_INTERNO", 	"cheque.ingresos");	//cheques propios cambiado como efectivo
...
/* Personas: constantes */
define("PERSONAS_TITULO_PARTES",		"Datos de Familiares");
...
define("CAPITAL_SOCIAL_EN_CAPTACION", 	(bool) $xC->get("manejar_captal_social_en_captacion", "false", MMOD_CAPTACION));
...
define("AML_MINIMO_EXCESO_EN_OPERACIONES_POR_PERFIL", $xC->get("minimo_de_exceso_en_operaciones_segun_perfil", 500, MMOD_AML) );
...
define("CAPTACION_PRODUCTO_ORDINARIO",		1);



define("TASA_ISR_POR_INTERESES", 		$xC->get("tasa_isr_por_intereses") );
define("SALARIOS_EXENTOS_ISR_INTERESES", 	$xC->get("salarios_minimos_exentos_por_isr_por_intereses") );
define("SALARIO_VIGENTE_DF", 			$xC->get("salario_vigente_en_el_df", "62.33", "entidad.legal") ); //Abril-2012 ;

define("INTERES_TIPO_CALCULO_DEFAULT", 		$xC->get("tipo_de_calculo_de_interes_por_defecto") );
//Cuadrar saldos de creditos y captacion VS Movimientos de operacion
define("FORCE_CUADRE_EN_OPERACIONES",		(bool) $xC->get("forzar_el_cuadre_de_documentos_por_movimientos")); //true
...
//============================================ RIESGOS
define("EDAD_PRODUCTIVA_MAXIMA",                $xC->get("edad_productiva_maxima") );
define("EDAD_PRODUCTIVA_MINIMA",                $xC->get("edad_productiva_minima") );
define("COSTO_CANASTA_BASICA",                  $xC->get("costo_de_la_canasta_basica") );
//Configuracion de seguimiento
define("DIAS_DE_INTERVALO_POR_LLAMADAS",        $xC->get("dias_de_intervalo_por_llamadas") );
define("DIAS_DE_ANTICIPACION_PARA_LLAMADAS",    $xC->get("dias_de_anticipacion_por_llamadas") );
define("DIAS_A_ESPERAR_POR_NOTIFICACION",       $xC->get("dias_a_esperar_por_notificacion") );
define("MINUTOS_A_ESPERAR_POR_LLAMADAS",        $xC->get("minutos_a_esperar_por_llamadas") );
...
class cConfiguration {
    ...
    function get($parametro, $mValor = false, $mTipo = "desconocido"){
        $sql    = "SELECT * FROM entidad_configuracion WHERE nombre_del_parametro ='$parametro' LIMIT 0,1";
        $cnn	= $this->init();
        
        $rs     = mysql_query($sql,  $cnn );
        if ( $rs == false ){
        	syslog(LOG_INFO, mysql_errno() . "|" . mysql_error() . "|"  . addslashes($sql) );
        }
        ...
        if ( $num == 0 ){
            ...
            $this->mValor   = trim( $dat["valor_del_parametro"] );
            if ( $this->mValor  === "true") {
                $this->mValor   = true;
            }
    ...
    function set($parametro, $valor){
        ...
        @mysql_unbuffered_query( $sql, $this->init() );
    }
    ...
    function init(){
		$cnn 	= mysql_connect(WORK_HOST, RPT_USR_DB, RPT_PWD_DB);
		$db 	= mysql_select_db(MY_DB_IN, $cnn);
        return $cnn;
    }
}
