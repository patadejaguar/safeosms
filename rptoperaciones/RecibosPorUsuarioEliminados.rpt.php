...
$ByFecha		= $xFil->RecibosArchPorFecha($FechaInicial, $FechaFinal);	
...
$archivo		= $xHP->getTitle();
	...
	$setSql = "SELECT
		...
		`operaciones_recibos_arch`.`idoperaciones_recibos`       AS `numero`,
		`operaciones_recibos_arch`.`fecha_operacion`             AS `fecha`,
		`operaciones_recibos_arch`.`numero_socio`                AS `socio`,
		`socios`.`nombre` ,
		`operaciones_recibos_arch`.`docto_afectado`              AS `documento`,
		`operaciones_recibostipo`.`descripcion_recibostipo` AS `tipo`,
		/*`operaciones_recibos_arch`.`cheque_afectador`            AS `cheque`,*/
		`operaciones_recibos_arch`.`tipo_pago`                   AS `forma_de_pago`,
		/*`operaciones_recibos_arch`.`recibo_fiscal`,*/
		/*`operaciones_recibos_arch`.`sucursal` ,*/
		`operaciones_recibos_arch`.`total_operacion`             AS `total`,
		`operaciones_recibos_arch`.`observacion_recibo` 
	FROM
		`operaciones_recibos_arch` `operaciones_recibos_arch` 
			INNER JOIN `socios` `socios` 
			ON `operaciones_recibos_arch`.`numero_socio` = `socios`.`codigo` 
				INNER JOIN `operaciones_recibostipo` `operaciones_recibostipo` 
				ON `operaciones_recibostipo`.`idoperaciones_recibostipo` = 
				`operaciones_recibos_arch`.`tipo_docto` 
					INNER JOIN `usuarios` `usuarios` 
					ON `operaciones_recibos_arch`.`idusuario` = `usuarios`.`idusuarios`
				
	WHERE  `operaciones_recibos_arch`.`idoperaciones_recibos` > 0
$ByFecha
		/*
		$ByUsuario
		$ByTipoPago
		...
		$ByEstad*/
	ORDER BY
		`operaciones_recibos_arch`.`fecha_operacion`,
		`operaciones_recibos_arch`.`tipo_pago`,
		`usuarios`.`idusuarios`,
		`operaciones_recibos_arch`.`numero_socio`
	";
