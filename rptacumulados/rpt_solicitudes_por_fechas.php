...
//=====================================================================================================
$xHP		= new cHPage("TR.MONTO DE CREDITOS SOLICITADOS", HP_REPORT);
$xL			= new cSQLListas();
$xF			= new cFecha();
$query		= new MQL();
...
$ByFechas		= $xFil->CreditosPorFechaDeSolicitud($FechaInicial, $FechaFinal);

$sql			= "SELECT
`creditos_tipoconvenio`.`descripcion_tipoconvenio` AS `producto`,
EnMiles(SUM(`creditos_solicitud`.`monto_solicitado`))       AS `solicitado`
FROM
`creditos_solicitud` `creditos_solicitud`
INNER JOIN `creditos_tipoconvenio` `creditos_tipoconvenio`
ON `creditos_solicitud`.`tipo_convenio` = `creditos_tipoconvenio`.
`idcreditos_tipoconvenio`
WHERE `creditos_solicitud`.`monto_solicitado` >0 $ByFechas
GROUP BY
`creditos_tipoconvenio`.`descripcion_tipoconvenio` ORDER BY `solicitado` DESC";
...
$xCh->addDataset($sql, "solicitado", "producto");
$xCh->setProcess();
$xRPT->addJsCode($xCh->getJs());
$xT->setFootSum(array(1=> "solicitado"));
