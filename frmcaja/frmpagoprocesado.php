...
$msg				.= "OK\tDATOS\tRecibiendo Datos Socio $socio, Credito $solicitud, Parcialidad $parcialidad, Periocidad $periocidad, Monto $monto_a_operar, Operacion $operacion \r\n";
...
$msg					.= "OK\tDATOS\tCheque $cheque, Recibo Fiscal $recibo_fiscal, Tipo de Pago $tipo_pago \r\n";
...
$xNRec->setNuevoRecibo($socio, $solicitud, $fecha_operacion, $parcialidad, RECIBOS_TIPO_PAGO_CREDITO, $observaciones,
		       $cheque, $tipo_pago, $recibo_fiscal, $grupo, false, AML_CLAVE_MONEDA_LOCAL, 0, $empresa);
$xNRec->init();
			...
			if($m[$id]!=0){
			  ...
			  if($periocidad == CREDITO_TIPO_PERIOCIDAD_FINAL_DE_PLAZO ){
			    ...
			    if($respetar_plan_pagos == true){
			      ...
			      $msg	.= "WARN\tPARCS\tEliminar Letra $id, $socio, $solicitud, $parcialidad, $recibo_pago\r\n";
			    } else {
			      setEliminarMvto360( $id, $socio, $solicitud, $recibo_pago);
...
//eliminar letra o neutralizarla en partes.
	...
	if ( $icls >= 1  ){
		...
		if ( $total_pendientes > TOLERANCIA_SALDOS ){
			...
			$msg	.= "WARN\tPARC_REST\tParcialidad regresada de $parcialidad a $nueva_parcialidad\r\n";
		}
	}
}
...
setNuevoMvto($socio, $solicitud, $recibo_pago, $fecha_operacion,  $baseM145, OPERACION_CLAVE_PAGO_CBZA, $parcialidad, $observaciones); //comision
...
$actualizar_parcialidad						= true;
	...
	if ($baseM120 > 0){
		...
		//calcular cambios de capital		//500 + ((4500 - 5000) [-500] *-1)[1000] / 5000 = 25%
											//1 - (500 / 4500 [0.1111])
		$proporcion_de_cambios				= 1 - ($baseM120 / $xCred->getSaldoActual()); //($baseM120  + ((($xCred->getSaldoActual() - $xCred->getMontoAutorizado()) ) * - 1) ) / $xCred->getMontoAutorizado(); 
