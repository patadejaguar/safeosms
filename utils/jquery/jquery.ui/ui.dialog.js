(function($)
{
	...
	$.ui.dialogInit = function(el, o) {
		...
		this.open = function() {
			...
			var wh = document.body.clientHeight, ww = document.body.clientWidth, doc = $(document), top = doc.scrollTop(), left = doc.scrollLeft();
			switch (options.position) {
				case 'center':
					top += (wh / 2) - (uiDialog.height() / 2);
					left += (ww / 2) - (uiDialog.width() / 2);
					break;
				case 'top':
					top += 0;
					left += (ww / 2) - (uiDialog.width() / 2);
				...
				case 'right':
					top += (wh / 2) - (uiDialog.height() / 2);
					left += (ww) - (uiDialog.width());
					break;
				case 'bottom':
					top += (wh) - (uiDialog.height());
					left += (ww / 2) - (uiDialog.width() / 2);
					break;
				case 'left':
					top += (wh / 2) - (uiDialog.height() / 2);
