...
$idsolicitud 		= (isset($_GET["pb"])) ? $_GET["pb"] : false;		//Numero de Solicitud
$id 				= (isset($_GET["pa"])) ? $_GET["pa"] : false;		//Numero de Socio
if(isset($_REQUEST["persona"])){
	$id			= $_REQUEST["persona"];
}
//Agosto-2013
$idsolicitud		= ( isset($_GET["credito"] ) ) ? $_GET["credito"] : $idsolicitud;

$f15 				= (isset($_GET["f15"])) ?$_GET["f15"] : "no";
$f14 				= (isset($_GET["f14"])) ?$_GET["f14"] : "no";
$f16 				= (isset($_GET["f16"])) ?$_GET["f16"] : "no";

$f18 				= (isset($_GET["f18"])) ?$_GET["f18"] : "no";		//Mostrar Movimiento Especifico
$TOperacion 		= (isset($_GET["f19"])) ?$_GET["f19"] : "no";		//Codigo de Tipo de Operacion.- Mvto Especifico
...
if ( $output != OUT_EXCEL ){
	...
	echo "<h2>ESTADO DE CUENTA DE CREDITOS</h2>";
  	...
  	$filename = "estado_de_cuenta-$idsolicitud-$id-" . date("YmdHi") . ".xls";
...
$xSQL				= new cSQLListas();
	...
	$solo_este_mvto 	= ($f18 == "yes") ? " AND operaciones_mvtos.tipo_operacion=$TOperacion " : "";
	...
	$sqlcred = "SELECT numero_socio, numero_solicitud,	saldo_actual, monto_autorizado	FROM creditos_solicitud	WHERE
		numero_solicitud=$idsolicitud	$solo_con_saldos	ORDER BY fecha_vencimiento DESC";
	...
	//
	if($id) {
		$sqlcred = "SELECT numero_socio, numero_solicitud, saldo_actual, monto_autorizado FROM creditos_solicitud WHERE numero_socio=$id $solo_con_saldos ORDER BY fecha_ministracion, fecha_vencimiento DESC";
		$idsocio = $id;
		$porSocio	= true;
	}
